...
public class Cuenta {
	...
	public Object depositar(Cuenta c, int monto) {
		...
		if(c.getMax()<monto) {
			...
			//Valida si el saldo actual + el monto excede el maximo permitido
		}else if((c.getSaldo()+monto)>c.getMax()) {
			return "Sobrepasa el maximo 2";
			//Si no, entonces se hace la transaccion
		}else {
	...
	public Object retirar(Cuenta c, int monto) {
		...
		if(c.getSaldo() < monto) {
			...
			return "El retiro dejaria sin fondos suficientes para la cuenta"; 
			}
			...
			System.out.println("Saldo actual en la cuenta ");
			return new Comprobante ("22 Enero 2008 ", direccion, c.getFolio(), monto); 
